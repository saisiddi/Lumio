...
export function PuzzleAnalysis({ progress }: { progress: number }) {
  ...
  useEffect(() => {
    ...
    const newPieces = Array.from({ length: GRID_SIZE * GRID_SIZE }).map(
      (_, i) => ({
        id: i,
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 opacity-20 pointer-events-none">
        <svg
          className="w-full h-full stroke-white/5 fill-none"
          viewBox="0 0 800 450"
        >
          <path
            d="M0,225 L800,225 M400,0 L400,450 M200,0 L200,450 M600,0 L600,450"
            strokeWidth="0.5"
          />
      ...
      <div
        className="relative grid gap-1.5 md:gap-2 p-4 md:p-6 w-[92%] h-[90%]"
        style={{
      ...
      >
        {pieces.map((piece) => {
          const isSolved =
            (piece.solveOrder / (GRID_SIZE * GRID_SIZE)) * 100 < progress;
          const { Icon } = piece;
              ...
              <div className="relative z-10 flex flex-col items-center gap-1">
                {piece.hasTree && isSolved ? (
                  <div className="w-8 h-8 flex items-center justify-center opacity-40">
                    <svg
                      className="w-full h-full stroke-current text-white/50"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path
                        d="M12 4L12 8 M12 8L6 14 M12 8L18 14 M6 14L4 18 M6 14L8 18 M18 14L16 18 M18 14L20 18"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                      />
                    </svg>
                  ...
                  <Icon
                    className={`w-5 h-5 transition-colors duration-500 ${isSolved ? "text-white" : "text-white/10"}`}
                  />
                )}

                <span
                  className={`text-[7px] md:text-[8px] font-mono font-black tracking-[0.16em] md:tracking-[0.2em] uppercase transition-colors duration-500 ${isSolved ? "text-white" : "text-white/5"}`}
                >
                  {piece.label}
                </span>
              </div>
      ...
      <div className="absolute bottom-4 md:bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 px-3">
        <div className="text-brand-electric font-mono text-[11px] md:text-sm tracking-[0.14em] md:tracking-widest uppercase font-bold text-center">
          Assembling Intelligence Matrix: {Math.round(progress)}%
        </div>
        <div className="w-56 md:w-64 h-1 bg-white/10 rounded-full overflow-hidden">
          <motion.div
